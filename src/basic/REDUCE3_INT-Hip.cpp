...
void REDUCE3_INT::runHipVariant(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      RAJA::ReduceSum<RAJA::hip_reduce<block_size>, Int_type> vsum(m_vsum_init);
      RAJA::ReduceMin<RAJA::hip_reduce<block_size>, Int_type> vmin(m_vmin_init);
      RAJA::ReduceMax<RAJA::hip_reduce<block_size>, Int_type> vmax(m_vmax_init);
