...
#define TRAP_INT_DATA_SETUP_HIP \
  Real_type x0 = m_x0; \
  Real_type xp = m_xp; \
  Real_type y = m_y; \
  Real_type yp = m_yp; \
                        ...
                        Index_type iend)
{
  ...
  if ( threadIdx.x == 0 ) {
    RAJA::atomic::atomicAdd<RAJA::atomic::hip_atomic>( sumx, psumx[ 0 ] );
  }
...
void TRAP_INT::runHipVariant(VariantID vid)
{
  ...
  if ( vid == Base_HIP ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; ++irep) {
      ...
      RAJA::ReduceSum<RAJA::hip_reduce<block_size>, Real_type> sumx(m_sumx_init);
